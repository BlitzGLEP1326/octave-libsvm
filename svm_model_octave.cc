 ...
 * This file contains functions for converting between an Octave structure and LIBSVM's model format.
...
const char *model_to_octave_structure(octave_value_list &plhs, int num_of_feature, struct svm_model *model)
{
	...
	for(i = 0; i < n; i++) cm_rho(i) = model->rho[i];
	osm_model.assign("rho", cm_rho);

	// Label
	if(model->label) {
		ColumnVector cm_label(model->nr_class);
		for(i = 0; i < model->nr_class; i++) cm_label(i) = model->label[i];
	...
	if(model->sv_indices)
	{
		...
		for(i = 0; i < model->l; i++) cm_sv_indices(i) = model->sv_indices[i];
	...
	if(model->probA != NULL)
	{
		...
		for(i = 0; i < n; i++) cm_proba(i) = model->probA[i];
	...
	if(model->probB != NULL)
	{
		...
		for(i = 0; i < n; i++) cm_probb(i) = model->probB[i];
	...
	if(model->nSV)
	{
		...
		for(i = 0; i < model->nr_class; i++) cm_nsv(i) = model->nSV[i];
